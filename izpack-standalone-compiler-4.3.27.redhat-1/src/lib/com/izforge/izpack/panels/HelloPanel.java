...
public class HelloPanel extends IzPanel
{
    ...
    public HelloPanel(InstallerFrame parent, InstallData idata, LayoutManager2 layout)
    {
        ...
        if (idata.info.getAppURL() != null)
        {
            ...
            // This has been changed from upstream so that websites are hyperlinked
            JLabel info = LabelFactory.create(str, parent.icons.getImageIcon("bookmark"), 0);
            JLabel appURLLabel = LabelFactory.create(idata.info.getAppURL(), idata.info.getAppURL().toString());
            JPanel p = new JPanel(new GridLayout(0, 2));
            p.setOpaque(!parent.hasBackground);
            p.add(info);
            p.add(appURLLabel);
            add(p, NEXT_LINE);
        }
