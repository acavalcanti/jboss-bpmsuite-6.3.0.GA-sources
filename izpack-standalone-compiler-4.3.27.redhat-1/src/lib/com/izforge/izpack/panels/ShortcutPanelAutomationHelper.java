...
public class ShortcutPanelAutomationHelper implements PanelAutomation
{
    ...
    public void runAutomated(AutomatedInstallData installData, IXMLElement panelRoot)
    {
        ...
        for (int i = 0; i < shortcutElements.size(); i++)
        {
            ...
            data.commandLine = vs.substitute(dataElement.getAttribute(ShortcutPanel.AUTO_ATTRIBUTE_COMMAND));
            data.description = dataElement.getAttribute(ShortcutPanel.AUTO_ATTRIBUTE_DESCRIPTION);
            data.iconFile = vs.substitute(dataElement.getAttribute(ShortcutPanel.AUTO_ATTRIBUTE_ICON));
            ...
            data.target = vs.substitute(dataElement.getAttribute(ShortcutPanel.AUTO_ATTRIBUTE_TARGET));
        ...
        // Adding this in to extract the file paths of the shortcuts for XDG use
        ...
        for (int i = 0; i < shortcuts.size(); i++)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // See init of startMenuShortcuts for expl.
                    if (OsVersion.IS_UNIX) {
                        // From lib/com/izforge/izpack/panels/ShortcutPanel.java, line 1550
                    ...
                    } // else not required - data made here will be used only on [LU]nix for XDG
        ...
        // Transposing functionality from ShortcutPanel.java -
        // XDG menu creation on [LU]nix
        if (OsVersion.IS_UNIX) {
            // Extract additional info from auto XML block (in general, shortcuts for all users or just current?)
            int shortcutsDestinationSelection;

            if (shortcut.getUserType() == Shortcut.ALL_USERS) {
                shortcutsDestinationSelection = Shortcut.ALL_USERS;
            } else { // otherwise just current user
                shortcutsDestinationSelection = Shortcut.CURRENT_USER;
            }

            String[] xdgFileNames = writeXDGMenuFile(startMenuShortcuts,
                                                     groupName,
                                                     "", //programGroupIconFile not provided by auto data
                                                     "", //programGroupComment not provided by auto data
                                                     shortcutsDestinationSelection);
        }
    ...
    // Methods copied from ShortcutPanel.java for XDG operations in automated mode
    ...
    private String[] writeXDGMenuFile(ArrayList desktopFileNames, String groupName,
                                      String icon, String comment, int shortcutsDestinationSelection)
    {
        ...
        if (shortcutsDestinationSelection == Shortcut.ALL_USERS)
        {
