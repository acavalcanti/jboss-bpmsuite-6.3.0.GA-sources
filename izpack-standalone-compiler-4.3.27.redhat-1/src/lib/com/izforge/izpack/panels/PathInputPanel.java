...
public class PathInputPanel extends IzPanel implements ActionListener {
    ...
    public boolean isValidated() {
        ...
        File newDir = null;
        String subDir = null;
        // show the message for creating a new directory or not.
        String vString = idata.getVariable("ShowCreateDirectoryMessage");
        final boolean vStr = vString != null ? Boolean.parseBoolean(vString) : true;
        if ((subDir = idata.getVariable("INSTALL_SUBPATH")) != null) {
        ...
        if (isMustExist()) {
            ...
            // Check if the subdirectory exists in the chosen path
            if (path.exists()) {
                if (newDir != null) {
                    if (newDir.exists()) {
                        ...
                        if (vStr) {
                            ok = this.emitNotificationFeedback(getI18nStringForClass(
                                    "createdir", "TargetPanel") + "\n" + chosenPath + File.separator + idata.getVariable("INSTALL_SUBPATH"));
                        }
                    }
                }
//              else {
