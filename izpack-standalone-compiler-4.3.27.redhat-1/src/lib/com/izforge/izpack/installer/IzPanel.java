...
public class IzPanel extends JPanel implements AbstractUIHandler, LayoutConstants {
    ...
    /**
     * Is this panel delayed or not (the next button will be deactivated for delayLength seconds if delay is set to true
     */
    protected boolean delay;

    /**
     * Is this panel going to show the "hidden" fields as disabled fields?
     */
    protected boolean displayHidden;

    /**
     * How long the delay is, in seconds
     */
    ...
    /**
     * Denotes whether this panel should appear in the summary panel or not
     */
    ...
    /**
     * Denotes whether this panel has been viewed
     */
    ...
    {
        super();
        if (parent.hasBackground) setOpaque(false);
        init(parent, idata);
        if (lm != null)
        {
    ...
    {
        parent.unlockNextButton(requestFocus);
    }
    ...
    {   
        if (isDelayed()){
            delayTimer = new Timer(delayLength, new ActionListener(){
    ...
    /**
     * Notify the user about something with links enabled in the message with a
     * standard sized dialog box
     */

    public void emitNotification(String message, boolean hasLinks){
        emitNotification(message, hasLinks, 400);
    }
    
    /**
     * Notify the user about something with links enabled in the message, and a
     * custom size for the given notification
     */
    public void emitNotification(String message, boolean hasLinks, int size) {
        ...
        // hacks to make the look&feel similar to a JLabel
        pane.setBackground(new JLabel().getBackground());
        Font font = getControlTextFont();
        String bodyRule = "body { font-family: " + font.getFamily() + "; " +
                "font-size: " + font.getSize() + "pt; }";
        ((HTMLDocument)pane.getDocument()).getStyleSheet().addRule(bodyRule);
        
        //pane.setFont(new JLabel().getFont());
        // TODO: reduce the hackishness. is proof of concept
    ...
    /**
     * Warn the user, and give them three options:
     * Continue, skip, cancel. 
     * The optionLabels is used to customize the button labels.
     *
     * @param titleId the string id for the message title
     * @param message the actual formatted message to display to the user
     */
    public int emitWarningWithSkip(String titleId, String message, Object [] optionLabels) {
       
        
        int ret = JOptionPane.showOptionDialog((Component) this,
                message,
                idata.langpack.getString(titleId),
    ...
    public int emitWarningWithSkip(String title, String message) {
        Object[] options = {"Continue",
                "Skip",
                "Cancel"};
        return emitWarningWithSkip(title, message, options);
    }
    ...
    public void loadFileChooserLang(){
        UIManager.put("FileChooser.openDialogTitleText", getI18nStringForClass("title", "BrowseDialog"));
        UIManager.put("FileChooser.saveDialogTitleText", getI18nStringForClass("title", "BrowseDialog"));
        UIManager.put("FileChooser.openButtonText", getI18nStringForClass("openButton","BrowseDialog"));
        UIManager.put("FileChooser.saveButtonText", getI18nStringForClass("openButton", "BrowseDialog"));
        UIManager.put("FileChooser.cancelButtonText", getI18nStringForClass("closeButton", "BrowseDialog"));
        UIManager.put("FileChooser.newFolderToolTipText", getI18nStringForClass("newButton", "BrowseDialog"));
        UIManager.put("FileChooser.renameFileButtonText", getI18nStringForClass("renameButton", "BrowseDialog"));
        UIManager.put("FileChooser.deleteFileButtonText", getI18nStringForClass("deleteButton", "BrowseDialog"));
        UIManager.put("FileChooser.openButtonToolTipText",getI18nStringForClass("openButtonTT", "BrowseDialog"));
        UIManager.put("FileChooser.saveButtonToolTipText", getI18nStringForClass("openButtonTT", "BrowseDialog"));
        UIManager.put("FileChooser.cancelButtonToolTipText", getI18nStringForClass("cancelButtonTT", "BrowseDialog"));
        UIManager.put("FileChooser.lookInLabelText", getI18nStringForClass("selectionText", "BrowseDialog"));
        UIManager.put("FileChooser.saveInLabelText", getI18nStringForClass("selectionText", "BrowseDialog"));
        UIManager.put("FileChooser.foldersLabelText",getI18nStringForClass("folderLabelText", "BrowseDialog"));
        UIManager.put("FileChooser.pathLabelText", getI18nStringForClass("selectionText", "BrowseDialog"));
        UIManager.put("FileChooser.folderNameLabelText", getI18nStringForClass("selectionText", "BrowseDialog"));
        UIManager.put("FileChooser.homeFolderToolTipText", getI18nStringForClass("homeButton", "BrowseDialog"));
        UIManager.put("FileChooser.upFolderToolTipText", getI18nStringForClass("upButton", "BrowseDialog"));
        UIManager.put("FileChooser.listViewButtonToolTipText", getI18nStringForClass("listButton", "BrowseDialog"));
        UIManager.put("FileChooser.detailsViewButtonToolTipText", getI18nStringForClass("detailsButton", "BrowseDialog"));
    }
    ...
    public String getSummaryCaption()
    {
        ...
        {
            if (this.getClass().getName().equals("com.izforge.izpack.panels.UserInputPanel")) {
                caption = getI18nStringForClass("summaryCaption."+this.getInstanceNumber(), this.getClass().getName());
            }
            else
    ...
    private final boolean validatePanel()
    {
        ...
        if (this.validationServices != null)
        {
            ...
            try
            {
                ...
                {
                    DataValidator.Status returnStatus = validationService
                            .validateData(this.idata);
                    ...
                    else
                    {
                        ...
                        if (returnStatus == DataValidator.Status.WARNING)
                        {
                            ...
                            if (this.emitWarning(getString("data.validation.warning.title"),
                                    vs.substitute(warningMessage, null)))
                            {
                                returnValue = true;
                        ...
                        } else if (returnStatus == DataValidator.Status.SKIP) {
                            /**
                             * If this validator implements skippable:
                             */
                            ...
                            if (SkippableDataValidator.class.isAssignableFrom(validationService.getClass())) {
                                ...
                                int userInput = emitWarningWithSkip(
                                        skipValidator.getSkipMessageId(),
                                        skipValidator.getFormattedMessage(),
                                        skipValidator.getSkipOptionLabels(idata));

                                Debug.trace("...running SKIP action.");
                                returnValue = skipValidator.skipActions(idata, userInput);
                            ...
                            if (errorMessage != null) {
                                emitError(getString("data.validation.error.title"), vs.substitute(errorMessage, null));
                            } else {
                                emitError(getString("data.validation.error.title"), "Validation Error");
                            }
                            returnValue = false;
                            break; // break if we fail. don't need to try other validators if one fails
                        }
                    }
                }
            }
    ...
    public boolean isDisplayingHidden() { return displayHidden; }
    ...
    protected final void executePreActivationActions()
    {
        if (preActivateActions != null)
        {
            ...
            {
                PanelAction action = preActivateActions.get(actionIndex);
                initActionPanel(action);
                action.executeAction(idata, this);
            }
        }
    }
    ...
    protected final void executePreValidationActions()
    {
        if (preValidateActions != null)
        {
            ...
            {
                PanelAction action = preValidateActions.get(actionIndex);
                initActionPanel(action);
                action.executeAction(idata, this);
            }
        }
    ...
    protected final void executePostValidationActions()
    {
        if (postValidateActions != null)
        {
            ...
            {
                PanelAction action = postValidateActions.get(actionIndex);
                initActionPanel(action);
                action.executeAction(idata, this);
            }
        }
