...
public class InstallerFrame extends JFrame {
    ...
    public InstallerFrame(String title, InstallData installdata, InstallerBase parentInstaller)
        ...
        loadIcons();
        loadCustomIcons();
        // This ensures panel button shortcuts don't override
        // installer frame button shortcuts.
        ButtonFactory.reserveInstallerFrameShortcuts(frameButtonNames());
        loadPanels();
        // This is necessary due to the order of GUI creation:
        // InstallerFrame is built after other panels.
    ...
    /**
     * Returns a localized list of button names for this installer frame.
     * @return
     */
    private String[] frameButtonNames()
    {
        ...
        String [] shortcuts = {
                   idata.langpack.getString("installer.quit"),
                   idata.langpack.getString("installer.previous"),
                   idata.langpack.getString("installer.next"),
                   idata.langpack.getString("installer.help")
                };
        return shortcuts;
    }
    ...
    private void loadPanels() throws Exception {
        ...
        IXMLElement productNameNode = new XMLElementImpl(AutomatedInstaller.PRODUCT_NAME_TAG, installData.xmlData);
        productNameNode.setContent(installData.getVariable("product.name").toUpperCase());
        installData.xmlData.addChild(productNameNode);

        IXMLElement productVersionNode = new XMLElementImpl(AutomatedInstaller.PRODUCT_VERSION_TAG, installData.xmlData);
        productVersionNode.setContent(installData.info.getAppVersion());
        installData.xmlData.addChild(productVersionNode);

        for (i = 0; i < size; i++) {
            // Clear out button mappings.
            ...
            installData.setVariable("panelID", p.getPanelid());
            object = constructor.newInstance(params);
            panel = (IzPanel) object;
            List<String> dataValidators = p.getValidators();
            if (dataValidators != null) {
                List<DataValidator> validationServices = new ArrayList<DataValidator>();
                for (String dataValidator : dataValidators){
                    validationServices.add(DataValidatorFactory.createDataValidator(dataValidator));
                }
    ...
    public class BackPanel extends JPanel {
        ...
        BackPanel () {
            try {
                image = (new ImageIcon(getClass().getResource("/res/Installer.background"))).getImage();
            } catch(Exception e){
                return;
            }
        ...
        public void paintComponent(Graphics g)
        {
            ...
            if(image != null) {
                g.drawImage(image, (this.getWidth()/2) - (image.getWidth(this) / 2),(this.getHeight()/2) - (image.getHeight(this) / 2),image.getWidth(this),image.getHeight(this),this);
            }
        }
    }
    ...
    private void buildGUI() {
        ...
        if (hasBackground) panelsContainer.setOpaque(false);
        ...
        if (hasBackground) navPanel.setOpaque(false);
        ...
        /*
         * AccessibileContext and ActionCommand setting for marathon automated testing
         */
        AccessibleContext ac = prevButton.getAccessibleContext();
        ac.setAccessibleDescription("This button moves the installation process back one panel");
        prevButton.setActionCommand("Previous Button");
        ac = nextButton.getAccessibleContext();
        ...
        ac = quitButton.getAccessibleContext();
    ...
    protected void switchPanel(int last) {
        ...
        try {
            ...
            // if processpanel not included in install.xml
            processPanel = null;
        }
        ...
        try {
            ...
            // Only the exit button in the last panel.
                ...
                lockNextButton();
            } else if (installdata.panels.get(installdata.curPanelNumber - 1) instanceof InstallPanel ||
                    processPanel != null && processPanel.isInstance(installdata.panels.get(installdata.curPanelNumber - 1))) {
                lockPrevButton();
            } else {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                ...
                public void run() {
                    ...
                    // Setting the next button to be the default even when
                    // it is disabled in order to avoid having the user
                    // accidently quit while rapidly pressing 'enter' through
                    // the installation panels.
                    getRootPane().setDefaultButton(nextButton);
                    Log.getInstance().addDebugMessage(
            ...
            panel.panelActivate();
            //quitButton.setFocusable(false); // Must set quitButton unfocusable before setting initial focus.
    ...
    private void writeUninstallData() {
        ...
        try {
            ...
            BufferedWriter logWriter = new BufferedWriter(new OutputStreamWriter(outJar, "UTF-8"));
     ...
     */
    public void exit() {
        IzPanel curPanel = installdata.panels.get(installdata.curPanelNumber);
        if ((installdata.canClose || ((!nextButton.isVisible() || !nextButton.isEnabled()) && (!prevButton.isVisible() || !prevButton.isEnabled())))
            && ((curPanel instanceof HTMLLicencePanel) || (curPanel instanceof FinishPanel))) {
            ...
            if (res == JOptionPane.YES_OPTION) {
                //set this variable so jobs registered in the housekeeper can be called only upon an aborted install
                this.installdata.setVariable("install.aborted", "true");

                /**
                 * We run the housekeeper's cleanUp methods *before* wiping all of the files because
                 * some of those methods rely on files in the installer tree. *gulp*
                 */
                Housekeeper.getInstance().runCleanup();

                /**
                 * Now we can safely delete all of the files the installer put down.
                 */
                wipeAborted();

                /**
                 * Finally we can shut down the whole production. Guilt-free.
                 */
                Housekeeper.getInstance().shutDownNoCleanUp(0);
    ...
    protected void wipeAborted() {
        ...
        /**
         * Remove root path only if it didn't exist prior to installation.
         */
        /*if (!Boolean.parseBoolean(idata.getVariable(ScriptParser.PRE_EXISING_INSTALL_PATH))) {
            File rootPath = new File(idata.getInstallPath());
            try {
    ...
    /**
     * Traverse the folder tree and remove any empty directories.
     * Directories containing files will be left alone.
     * @param rootPath
     */
    private void removeCleanFolders(String rootPath) {
        ...
        // Traverse through the folder tree and fill up
        // the queue with all the folders in the tree.
        while (!folderStack.isEmpty()) {
            thisFile = folderStack.pop();
        ...
        // Iterate through the queue and delete empty folders.
        // Reverse order to get all the children first.
        for (int i=folderQueue.size()-1; i>=0; i--) {
            if (folderQueue.get(i).listFiles().length<=0) {
                folderQueue.get(i).delete();
            }
        }
    }
    ...
    public void writeXMLTree(IXMLElement root, OutputStream out) throws Exception {
        ...
        for (int i = 0; i < installdata.panels.size(); i++) {
            if (canShow(i)) {
                ...
                //Add the plus 2 because we added the product version and name as the first two nodes
                panel.makeXMLData(installdata.xmlData.getChildAtIndex(i+2));
            }
        }
        writer.write(root);
    ...
    public void navigatePrevious(int endingPanel) {
        ...
        if (-1 != prevPanel) {
            final IzPanel panel = installdata.panels.get(installdata.curPanelNumber);
