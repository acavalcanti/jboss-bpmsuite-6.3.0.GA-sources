...
public class ProcessPanelWorker implements Runnable
{
    ...
    private ArrayList<ProcessingJob> onFailJobs = new ArrayList<ProcessingJob>();

    private ArrayList<ProcessingJob> finalJobs = new ArrayList<ProcessingJob>();
    ...
    private boolean successfulInstall = true;
    ...
    private boolean readSpec() throws IOException
    {
        ...
        for (IXMLElement job_el : spec.getChildrenNamed("job"))
        {
            ...
            /**
             * Parse the extra attributes: 'final' means this job is always run at the end. 'onfail'
             * means this job is run right after a failed job, but before 'final' jobs.
             */
            boolean jobIsFinal = isFinal(job_el);
            boolean jobIsOnFail = isOnFail(job_el);
            ...
            if (OsConstraint.oneMatchesCurrentSystem(constraints))
            {
                ...
                /**
                 * Now that we've passed all of the conditions and requirements hurdles, we can add
                 * the job to the appropriate queue:
                 */
                if (jobIsFinal)
                {
                ...
                {
                    this.onFailJobs.add(new ProcessingJob(job_name, ef_list));
                }
                else
                {
    ...
    /**
     * Checks out a job to see if it's a 'final' job: that is, it should be run after all other jobs
     * regardless of failure or success.
     * 
     * @param job_el
     * @return
     */
    private boolean isFinal(IXMLElement job_el)
    {
        return isAttributeTrue(job_el, "final", false);
    }

    /**
     * Checks out if a job should be run after a failed job.
     * 
     * @param job_el
     * @return
     */
    private boolean isOnFail(IXMLElement job_el)
    {
        return isAttributeTrue(job_el, "onfail", false);
    }

    /**
     * Tests if this xml element contains a given attribute, and whether the value is set to true or
     * not. If the atty isn't present, returns the defaultValue.
     * 
     * @param ele An IXMLElement to test.
     * @param atty A String: The name of the attribute.
     * @param defaultValue: A boolean: The default value.
     * @return
     */
    private boolean isAttributeTrue(IXMLElement ele, String atty, boolean defaultValue)
    {
        return ele.hasAttribute(atty) ? Boolean.parseBoolean(ele.getAttribute(atty)) : defaultValue;
    }
    ...
    public void run()
    {
        ...
        this.handler.startProcessing(this.jobs.size() + this.finalJobs.size());

        /**
         * This is the loop that will actually run the jobs. Not so bad!
         */
        for (ProcessingJob pj : this.jobs)
        {

            this.result = runJob(pj);

            /**
             * If the job fails, we exit prematurely.
             */
            if (!this.result)
            {
                /**
                 * But not before running the onfail jobs.
                 */
                for (ProcessingJob ofj : this.onFailJobs)
                {
                    // we don't care about the onfail job return values.
                    runJob(ofj);
                }
        ...
        /**
         * This is where we run the final jobs that always run, provided they passed their
         * conditions etc.
         */
        for (ProcessingJob fj : this.finalJobs)
        {
            this.result = runJob(fj);
        }

        boolean unlockNext = true;
        /**
         * We just ran all of the post-install jobs, thereby making all sorts of messes. You can't
         * go back now, son!
         */
    ...
    /**
     * @return the successfulInstall parameter
     */
    public boolean getSuccessfulInstall(){
        return this.successfulInstall;
    }

    /**
     * Runs a job as a new process, captures its return value, finishes the process, then returns
     * the value.
     * 
     * @param pj
     * @return the return value from the ProcessingJob.
     */
    private boolean runJob(ProcessingJob pj)
    {
        this.handler.startProcess(pj.name);

        boolean val = pj.run(this.handler, this.vs);
    ...
    private boolean jobRequiredFor(Vector<IXMLElement> packs)
    {
        ...
        {
            selected = ((Pack) idata.selectedPacks.get(i)).id;

            // System.out.println ("Selected pack is " + selected);

            for (int k = 0; k < packs.size(); k++)
            {	
            	// thauser: change to use the ID instead of the name. 
                required = (packs.elementAt(k)).getAttribute("id", "");
