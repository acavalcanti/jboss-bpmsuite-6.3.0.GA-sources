...
public class InstallerBase
{
    ...
    /**
     * TODO: Make this a parameter that we parse from install.xml
     */
    protected static String LANG_BACKUP = "eng";

    public static String getLangBackup()
    {
        return LANG_BACKUP;
    }

    public static void setLangBackup(String backup)
    {
        LANG_BACKUP = backup;
    }
    ...
    public void loadInstallData(AutomatedInstallData installdata) throws Exception
    {
        ...
        for (i = 0; i < size; i++)
        {
            ...
            if (OsConstraint.oneMatchesCurrentSystem(pk.osConstraints))
            {
                ...
                AutomatedInstallData.availablePacksMap.put(pk.id, pk);
            }
        }
        objIn.close();
    ...
    /**
     * Loads variable key value pairs from the command line into AutomatedInstallData.
     * @param argVariables
     */
    public void loadCommandLineVariables(Map<String,String> argVariables, AutomatedInstallData idata) {
        if (argVariables != null) {
            for (Map.Entry<String,String> entry : argVariables.entrySet()) {
    ...
    protected void addBackupLangpack(AutomatedInstallData idata)
    {
        try
        {
            InputStream in = getClass().getResourceAsStream(
                    "/res/" + LANG_FILE_NAME + "_" + LANG_BACKUP);
            idata.langpack.add(in);
        }
        catch (Exception e)
        {
            Debug.trace("No backup langpack available.");
        }
    }
