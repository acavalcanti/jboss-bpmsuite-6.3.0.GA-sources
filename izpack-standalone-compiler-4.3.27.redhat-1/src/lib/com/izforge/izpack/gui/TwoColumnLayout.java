...
public class TwoColumnLayout implements LayoutManager2
{
	...
	private boolean overlapAdjustment = false; // added to the y component each time there is a component overlap. can get confusing
	...
	private void positionTitle(Container parent)
	{
		if (title != null)
		{
			...
			if (component != null)
			{
				...
				{
                    // HACKS: put it in the center of the panel, not the midpoint of the two columns
	...
	private void positionComponent(int y, int row, int column, Container parent)
	{
		...
		if (constraints != null)
		{
			...
			{
				/*
				 * Account for overlap. If overlap is set, we place this component overtop of the previous one.
				 * 
				 */
				if (constraints.overlap){
					// need to calculate y by using the previous component's y values.
                    int offset;
                    if (column == LEFT) {
                        offset = (int)Math.round(height * 1.5);
                    } else {
                        offset = height;
                    }
                    y = y - offset;
					overlapAdjustment = true;
				}
				...
				{
				    // don't subtract indent from width, because it results in weird stuff when indenting labels. 
				    // added May 30 2013, if this causes any strange userInputBugs, don't hesitate to create a better solution, perhaps
				    // dependent on the type of component
					//width -= indent;
					x += constraints.indent;
				}
	...
	/**
	 * Returns the minimum width of the column requeshted.
	 *
