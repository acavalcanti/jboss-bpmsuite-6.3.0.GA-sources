...
public class Unix_Shortcut extends Shortcut implements Unix_ShortcutConstants
{
        ...
        if ("".equals(this.itsGroupName) && (this.getLinkType() == Shortcut.DESKTOP))
        {
            ...
            if (xdgDesktopIconCmd != null)
            {
                ...
                myInstallScript.appendln(new String[] { myXdgDesktopIconCmd, "install",
                        "--novendor", formatPathForBash(StringTool.escapeSpaces(writtenDesktopFile.toString()))});                
    ...
    private String formatPathForBash(String aPathString) {
        List<String> replaceWhat = Arrays.asList(new String[] {"]","[","}","{","(",")","<",">"});
        StringBuffer returnVal = new StringBuffer(aPathString.length()*2);
        for (char ltr : aPathString.toCharArray()){
            if (replaceWhat.contains(String.valueOf(ltr)))
                returnVal.append("\\"+ltr);
            else
                returnVal.append(ltr);
        }
	...
	//delete existing shortcuts from previous installs that may still exist 
	String SCname = (replaceSpacesWithMinus == true ? StringTool
                            .replaceSpacesWithMinus(shortcutName) : shortcutName);
	File directory = new File (targetPath);
	String[] existingSCs = directory.list ();
	if (existingSCs != null) {
		for (String SC: existingSCs) {
			if (SC.contains(SCname)) {
				try {
					BufferedReader br = new BufferedReader(new InputStreamReader(new DataInputStream(new FileInputStream(targetPath+SC))));
					String line  = null;
					while( ( line = br.readLine() ) != null && !line.startsWith("Exec") );
					if (shortcutDef.contains(line)) {
						File toDelete = new File (targetPath+SC);
