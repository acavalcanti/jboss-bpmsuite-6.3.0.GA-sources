...
public class IoHelper
{
    ...
    public static File existingParent(File path)
    {
        ...
        while (!result.exists())
        {
            ...
            {
                String pathString = result.getAbsolutePath();

                if(pathString.matches("[a-zA-Z]:\\\\.*"))
                {
                    String drive = pathString.substring(0, 3);
                    File driveFile = new File(drive);
                    File [] roots = result.listRoots();

                    if (!Arrays.asList(roots).contains(driveFile))
                    {
                        return null;
                    }
                }
    ...
    /**
     * Replaces first instance of ~ with user's home directory on unix platform
     * @param path file path
     * @return return parsed path
     */
    public static String expandHomePath(String path) {
        if (path.startsWith("~") && OsVersion.IS_UNIX)
        {
   ...
   /**
    * Checks whether the remote file hosted at name exists.
    * @param 
    */
