...
public final class OsVersion implements OsVersionConstants, StringConstants
{
    ...
    public static final boolean IS_REDHAT_LINUX = IS_LINUX
            && ((FileUtil.fileContains(getReleaseFileName(REDHAT), REDHAT) || FileUtil.fileContains(getReleaseFileName(REDHAT),
            RED_HAT)));
     ...
     */
    private static String getReleaseFileName(String os)
    {
        String result = "";
        os = os.toLowerCase();
        ...
        if (etcList != null)
        {
            for (File etcEntry : etcList)
            {
                if (etcEntry.isFile())
                {
                    ...
                    else if(etcEntry.getName().toLowerCase().startsWith(os))
                    {
                        return result = etcEntry.toString();
                    }
                }
    ...
    private static String getLinuxDistribution()
    {
        ...
        if (IS_SUSE_LINUX)
        {
            try
            {
                result = SUSE + SP + LINUX + NL + StringTool.stringArrayListToString(FileUtil.getFileContent(getReleaseFileName(SUSE)));
            }
            catch (IOException e)
            {
                // TODO ignore
        ...
        else if (IS_REDHAT_LINUX)
        {
            try
            {
                result = REDHAT + SP + LINUX + NL + StringTool.stringArrayListToString(FileUtil.getFileContent(getReleaseFileName(REDHAT)));
            }
            catch (IOException e)
            {
                // TODO ignore
        ...
        else if (IS_FEDORA_LINUX)
        {
            try
            {
                result = FEDORA + SP + LINUX + NL
                        + StringTool.stringArrayListToString(FileUtil.getFileContent(getReleaseFileName(FEDORA)));
            }
            catch (IOException e)
            {
                // TODO ignore
        ...
        else if (IS_MANDRAKE_LINUX)
        {
            try
            {
                result = MANDRAKE + SP + LINUX + NL
                        + StringTool.stringArrayListToString(FileUtil.getFileContent(getReleaseFileName(MANDRAKE)));
            }
            catch (IOException e)
            {
                // TODO ignore
        ...
        else if (IS_MANDRIVA_LINUX)
        {
            try
            {
                result = MANDRIVA + SP + LINUX + NL
                        + StringTool.stringArrayListToString(FileUtil.getFileContent(getReleaseFileName(MANDRIVA)));
            }
            catch (IOException e)
            {
                // TODO ignore
        ...
        else
        {
            try
            {
                result = "Unknown Linux Distribution\n"
                        + StringTool.stringArrayListToString(FileUtil.getFileContent(getReleaseFileName(UNKNOWN)));
            }
            catch (IOException e)
            {
                // TODO ignore
    ...
    public static String getOsDetails()
    {
        ...
        if (IS_UNIX)
        {
            ...
            else
            {
                try
                {
                    result.append(FileUtil.getFileContent(getReleaseFileName(UNKNOWN))).append(NL);
                }
                catch (IOException e)
                {
